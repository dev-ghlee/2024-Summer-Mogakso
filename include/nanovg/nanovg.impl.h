...
void nvgReset(NVGcontext* ctx)
{
	...
	state->fontId = 0;
	// Joony
	state->outputGamma[0] = state->outputGamma[1] = state->outputGamma[2] = 2.4f;
	state->outputMat[0] = state->outputMat[4] = state->outputMat[8] = 1.f;
	state->outputMat[1] = state->outputMat[2] = state->outputMat[3] = 0.f;
	state->outputMat[5] = state->outputMat[6] = state->outputMat[7] = 0.f;
	// End Joony
}
...
void nvgFill(NVGcontext* ctx)
{
	...
	ctx->params.renderFill(ctx->params.userPtr, &fillPaint, state->compositeOperation, &state->scissor,
						   state->outputGamma, state->outputMat,
						   ctx->fringeWidth,
...
// Joony
void nvgOutputGamma(NVGcontext* ctx, const float* gamma ) {
	NVGstate* state = nvg__getState(ctx);
	state->outputGamma[0] = gamma[0];
	state->outputGamma[1] = gamma[1];
	state->outputGamma[2] = gamma[2];
}

void nvgOutputColorMat(NVGcontext* ctx, const float* mat ) {
	NVGstate* state = nvg__getState(ctx);
	state->outputMat[0] = mat[0];
	state->outputMat[1] = mat[1];
	state->outputMat[2] = mat[2];
	state->outputMat[3] = mat[3];
	state->outputMat[4] = mat[4];
	state->outputMat[5] = mat[5];
	state->outputMat[6] = mat[6];
	state->outputMat[7] = mat[7];
	state->outputMat[8] = mat[8];
}
// End Joony
