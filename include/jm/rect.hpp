...
template<typename T> struct tdim2 {
	...
	inline	mvec2_t<T>	operator * (const mvec2_t<T>& v) const { return mvec2_t<T>(w*v.x,h*v.y); }
...
/// \brief Heterogeneous division between tdim2 and scalar, which scales the size of tdim2
/// Multiplication of idim2, d and float, s produce dim2(d.w*s, d.h*s).
/// @param d The idim2 to scale.
/// @param s The scale facor.
/// @return The dim2 with the minimum width and the minimum height.
...
template<typename T> struct trect {
	union {
		...
		struct {
			mvec2_t<T> p;	///< The top-left corner of the rectangle.
	...
	/// @param ss The size of the rectangle.
	inline				trect( const mvec2_t<T>& pp, const tdim2<T>& ss )
	: p(pp), sz(ss) {}
	...
	/// @param c The width of the rectangle.
	/// @param d The height of the rectangle.
	inline				trect( const mvec2_t<T>& pp, const T& c=0, const T& d=0 )
	...
	inline				trect( const mvec2_t<T>& a, const mvec2_t<T>& b )
	...
	inline mvec2_t<T>	tl() const { return p; }
	
	/// The top-right corner of the rectangle.
	inline mvec2_t<T>	tr() const { return mvec2_t<T>(x+w,y); }

	/// The bottom-left corner of the rectangle.
	inline mvec2_t<T>	bl() const { return mvec2_t<T>(x  ,y+h); }

	/// The bottom-right corner of the rectangle.
	inline mvec2_t<T>	br() const { return mvec2_t<T>(x+w,y+h); }

	/// The size of the rectangle.
	inline tdim2<T>		wh() const { return tdim2<T>(w,h); }

	/// The top-left corner of the rectangle.
	/// Note that this variation returns the assignable (reference) mvec2_t<T>.
	/// return The assignable top-left corner of the rectangle
	inline mvec2_t<T>&	tl() { return p; }
	...
	inline bool			in( const mvec2_t<T>& p ) const { return (p.x>=x)&&(p.y>=y)&&(p.x<x+w)&&(p.y<y+h); }
	...
	inline trect&		increase( const mvec2_t<T>& p ) {
...
template<typename T>
T const* value_ptr(trect<T> const& v) { return &(v.x); }
template<typename T>
T* value_ptr(trect<T>& v) { return &(v.x); }
